...
import { useStore, TransactionType, PaymentMethod, PAYMENT_METHODS, Transaction } from '@/store/useStore';
...
import { addTransactionRemote, updateTransactionRemote } from '@/lib/sync';
...
export default function TransactionSheet({ open, onOpenChange, editingTransaction }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTransaction) {
        const result = await updateTransactionRemote(editingTransaction.id, txnData);
        if (!result.success) throw new Error(result.error);
        toast.success('Transaction updated');
      } else {
        const result = await addTransactionRemote(txnData);
        if (!result.success) throw new Error(result.error);
